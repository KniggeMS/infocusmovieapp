...
import { Search, Plus, Trash2, Heart, Eye, Shield, Film, ChevronDown, ChevronUp } from 'lucide-react';
...
function App({ conductor }: AppProps) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = conductor.subscribe((newState: WatchlistState) => {
      setState(newState);
    });
  ...
  return (
    <div className="app-container">
      ...
      {/* Header */}
      <header className="glass-header sticky top-0 z-40 px-4 py-3 flex items-center gap-3">
        <GlassInput
          value={searchTerm}
      ...
      <main className="px-3 pt-3 pb-24">
        ...
        {state.status === 'loading' && (
          <div className="text-center py-4 text-app-text-muted text-sm animate-pulse">
            {t('common.loading')}
          </div>
        )}
        ...
        {state.filter === 'diary' ? (
          ...
          <EpisodeTracker
            ...
            onToggleEpisode={(showId: number, season: number, episode: number) =>
          ...
          <>
            ...
            {allTags.length > 0 && (
              <div className="mb-3">
                ...
                >
                  <span>Tags</span>
                  {state.tagFilter && (
                    <span className="w-4 h-4 rounded-full bg-accent-color text-white text-[9px] flex items-center justify-center">
                      1
                    </span>
                  )}
              ...
              {filteredItems.map((movie) => (
                ...
                >
                  {movie.posterPath ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-app-secondary text-app-text-muted">
                      <Film size={24} />
                    </div>
                  )}

                  {movie.watched && (
                    <span className="absolute top-1 left-1 bg-green-500/80 backdrop-blur-sm text-white text-[10px] px-1.5 py-0.5 rounded-full">
                      ✓ Gesehen
                    </span>
                  )}

                  {movie.voteAverage && (
                    <span className="absolute top-1 right-1 bg-black/60 backdrop-blur-sm text-yellow-400 text-[10px] px-1.5 py-0.5 rounded-full">
                  ...
                  <div className="absolute bottom-8 right-1 flex flex-col gap-1">
                    {movie.source !== 'tmdb' && (
                      <>
                        ...
                        >
                          <Heart size={13} className={movie.favorite ? 'fill-red-500 text-red-500' : 'text-white'} />
                        </button>
                        <button
                          onClick={(e) => { e.stopPropagation(); conductor.dispatch({ type: 'TOGGLE_WATCHED', payload: movie.id }); }}
                          className="bg-black/60 backdrop-blur-md p-2 rounded-full transition-all hover:scale-110 shadow-lg"
                        >
                          <Eye size={13} className={movie.watched ? 'fill-green-500 text-green-500' : 'text-white'} />
                        </button>
                      </>
                    )}
                  ...
                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent p-2 pt-6">
                    ...
                    <div className="flex items-center gap-1 mt-0.5 flex-wrap">
                      ...
                      <span className="text-white/60 text-[10px]">{movie.releaseDate?.split('-')[0] || 'N/A'}</span>
                    </div>
            ...
            {state.status === 'idle' && state.items.length === 0 && !state.error && (
              <div className="flex flex-col items-center justify-center py-20 text-center gap-3">
                ...
                <p className="text-app-text-muted font-medium">{t('common.noResults')}</p>
                <p className="text-app-text-faint text-sm">{t('common.beginSearch')}</p>
              </div>
            )}
          </>
        )}
