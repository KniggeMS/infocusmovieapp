...
export class SupabaseMovieService implements MovieServiceAdapter {
  ...
  async getById(id: string): Promise<Movie | null> {
    ...
    try {
      const { data, error } = await this.client.from('movies').select('*').eq('id', id).single();
  ...
  async getMovieDetails(tmdbId: string, mediaType: 'movie' | 'tv' = 'movie'): Promise<Movie> {
    ...
    try {
      ...
      const director =
        data.credits?.crew?.find((m: any) => m.job === 'Director' || m.job === 'Executive Producer')
          ?.name || 'Unknown';
  ...
  async delete(id: string): Promise<void> {
    ...
    const { error } = await this.client.from('movies').delete().eq('id', id);
    ...
    if (error) {
      ...
      if (/column .* does not exist|user_rating|notes|tags/i.test(msg)) {
        ...
        if (Object.keys(safeUpdate).length === 0) {
          console.warn(
            'Skipping update — only optional columns were requested but column is missing:',
            msg,
          );
          return;
        }
  ...
  async getListMovies(listId: string): Promise<Movie[]> {
    const { data, error } = await this.client.from('list_items').select('*').eq('list_id', listId);
  ...
  async addMovieToList(listId: string, movie: Movie): Promise<void> {
    ...
    const { error } = await this.client.from('list_items').insert({
      list_id: listId,
  ...
  async saveEpisodeProgress(episodes: EpisodeEntry[]): Promise<void> {
    ...
    for (const ep of episodes) {
      ...
      const { error } = await this.client.from('tv_progress').insert({
        user_id: user.id,
        tmdb_id: ep.tmdbId,
  ...
  async saveDiaryEntry(entry: Movie): Promise<void> {
    ...
    const { error } = await this.client.from('diary_entries').insert({
      user_id: user.id,
      tmdb_movie_id: tmdbId,
  ...
  async loadAchievements(): Promise<{ achievementId: string; unlockedAt: string }[]> {
    const {
      data: { user },
    } = await this.client.auth.getUser();
    if (!user) return [];
    ...
    if (error) {
      console.warn('loadAchievements error:', error.message);
      return [];
    }
  ...
  async saveAchievement(achievementId: string): Promise<void> {
    ...
    if (error) {
      console.warn('saveAchievement error:', error.message);
    }
  }
}
