        ...
        <div className="relative w-full max-w-7xl bg-app-bg sm:rounded-3xl border-t sm:border border-app-border shadow-2xl h-[95vh] sm:h-[90vh] overflow-y-auto overflow-x-hidden animate-slide-up scrollbar-hide">
          ...
          <div className="px-4 sm:px-8 pb-6 space-y-5 sm:space-y-6 bg-app-bg relative z-30 -mt-0">
            ...
            {movie.source === 'database' && (
              <PersonalSection movie={movie} conductor={conductor} onShowToast={onShowToast} />
            )}
      ...
      <div className="px-5 sm:px-10 pt-1 pb-2 relative z-30">
        <div className="flex flex-wrap items-center gap-2 sm:gap-3">
          {movie.trailerKey ? (
            <button
              onClick={() =>
                window.open(`https://youtube.com/watch?v=${movie.trailerKey}`, '_blank')
              }
            ...
            >
              ...
              {t('common.searchTrailer', 'Trailer suchen')}
            </button>
          )}
...
const MetadataGrid = React.memo(({ movie }: { movie: Movie }) => {
  ...
  const items = useMemo(
    ...
    [movie.director, movie.genres, movie.releaseDate, t],
  );
  ...
  ({
    ...
    const persistRating = useCallback(
      (value: number | null) => {
        ...
        conductor.dispatch({
          ...
          payload: { id: movie.id, userRating: value },
        });
      },
      [conductor, movie.id],
    );
    ...
    return (
      <section className="bg-app-secondary/30 border border-app-border rounded-2xl p-4 sm:p-5 space-y-5">
            ...
            {ratings.tmdb !== null && (
              ...
              >
                <img
                  ...
                  className="h-3"
                />
                <span className="text-xs font-bold text-white">{ratings.tmdb.toFixed(1)}</span>
              </a>
            )}
        ...
        {/* Notes */}
        <div>
          <div className="text-xs text-app-text-muted uppercase tracking-wider mb-2">
            {t('common.note', 'Notiz')}
          </div>
          <textarea
            value={notes}
            ...
            placeholder={t(
              ...
              'Was soll dir an diesem Film im Kopf bleiben?',
            )}
            rows={3}
