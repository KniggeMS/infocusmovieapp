        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2 mt-6 text-sm text-app-text-muted">
              {mode !== 'login' && (
                <button type="button" onClick={() => setMode('login')} className="hover:text-app-text transition-colors underline underline-offset-2">
                  {t('auth.hasAccount')} {t('auth.login')}
                </button>
              )}
              {mode !== 'signup' && (
                <button type="button" onClick={() => setMode('signup')} className="hover:text-app-text transition-colors underline underline-offset-2">
                  {t('auth.noAccount')} {t('auth.signup')}
                </button>
              )}
              {mode !== 'forgot' && (
                <button type="button" onClick={() => setMode('forgot')} className="hover:text-app-text transition-colors underline underline-offset-2 text-xs">
