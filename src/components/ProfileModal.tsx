...
const themes = [
  {
    id: 'noir',
    name: 'Noir',
    class: 'bg-[#050505] border-white/20',
  ...
  },
  {
    id: 'glass',
    name: 'Glass',
    class: 'bg-[#0f172a] border-blue-400/30',
  ...
  },
  {
    id: 'neon',
    name: 'Neon',
    class: 'bg-[#0d0221] border-purple-500/50',
    preview: 'from-purple-900 to-[#0d0221]',
...
export const ProfileModal = React.memo(
  ({ user, conductor, customLists, onClose, onLogout, onUpdateUser }: ProfileModalProps) => {
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6 space-y-6 scrollbar-hide">
            ...
            <GlassSection title="Erscheinungsbild" icon={<Monitor className="w-3.5 h-3.5" />}>
                  ...
                  <motion.button
                    ...
                    className={`relative h-20 rounded-xl border-2 transition-all flex flex-col items-center justify-center gap-1 overflow-hidden ${t.class} ${
                      (user.theme || 'noir') === t.id
                        ? 'ring-2 ring-accent-color ring-offset-2 ring-offset-app-bg'
                        : 'opacity-75 hover:opacity-100'
                    }`}
                  >
                    <div className={`absolute inset-0 bg-gradient-to-br ${t.preview} opacity-50`} />
                    <span className={`relative z-10 text-[10px] font-bold ${t.textClass}`}>
                      {t.name}
                    </span>
                    {(user.theme || 'noir') === t.id && (
                      <motion.div
              ...
              >
                ...
                <div
                  className={`ml-auto w-9 h-5 rounded-full transition-colors ${followSystem ? 'bg-accent-color' : 'bg-app-secondary'}`}
                >
                  <motion.div
            ...
            <GlassSection title="Benutzer">
              <div className="flex items-center gap-4 mb-4">
                ...
                <div className="min-w-0">
                  <h3 className="text-lg sm:text-xl font-bold text-app-text truncate">
              ...
              <GlassCard className="p-0 divide-y divide-glass-border">
                <div className="flex items-center justify-between p-3">
                  ...
                  {editingUsername ? (
                    ...
                    >
                      <strong className="text-sm text-app-text truncate max-w-[150px]">
                        {user.username || '—'}
                      </strong>
                      <span className="text-[10px] text-app-text-muted group-hover:text-blue-400 uppercase tracking-wider opacity-0 group-hover:opacity-100 transition-opacity">
                        ✎
                      </span>
                    </button>
                  )}
                ...
                <div className="grid grid-cols-4 gap-2 text-center">
                  {[
                    ...
                    <div key={label} className="p-2">
                      ...
                      <div className="text-[9px] text-app-text-muted uppercase tracking-wider truncate">
                        {label}
                      </div>
                    </div>
                  ))}
...
function RoleBadge({ role }: { role: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-bold uppercase tracking-wider ${styles[role] || 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}
    >
      {icons[role]} {role}
    </span>
  );
